(function() {
	...
	module.exports = new Class({
         ...
         */
        , analyze: asyncMethod(function(schemas, callback) {
            // schemas, datbase = this.options.database;

            var config = this.connection.getConfig();

            Promise.all(schemas.map(function(schemaName) {
                            ...
                            constraint.rules.forEach(function(rule){
                                switch (constraint.type) {
                                    ...
                                    case 'foreign key':
                                        ...
                                        database[tableName].columns[rule.column_name].updateAction = rule.on_update.toLowerCase();
                                        database[tableName].columns[rule.column_name].deleteAction = rule.on_delete.toLowerCase();
            ...
            return Promise.all([{
                  SQL: `        SELECT tc.table_name, 
                                       ...
                                       (
                                        ...
                                        (
                                                  SELECT rc.update_rule on_update
                                                    FROM information_schema.referential_constraints rc
                                                   WHERE rc.constraint_catalog = tc.constraint_catalog
                                                     AND rc.constraint_schema = '${databaseName}'
                                                     AND rc.constraint_name = tc.constraint_name
                                        ) on_update,
                                        (
                                                  SELECT rc.delete_rule on_delete
                                                    FROM information_schema.referential_constraints rc
                                                   WHERE rc.constraint_catalog = tc.constraint_catalog
                                                     AND rc.constraint_schema = '${databaseName}'
                                                     AND rc.constraint_name = tc.constraint_name
        ...
        , listTables: function(databaseName) {
            return this.connection.query({SQL: 'SELECT table_schema,table_name FROM information_schema.tables WHERE table_schema = '+databaseName+' ORDER BY table_schema,table_name;', mode: 'query'});
        }
